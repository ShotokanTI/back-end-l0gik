...
export const createSeries = async (
  ...
  if(!result){
    const newUser = await getRepository(Series).create(req.body);
    const results = await getRepository(Series).save(newUser).then(saved=>{
...
export const updateSeries = async (
  ...
  const user = await getRepository(Series).findOne(req.params.id);
  if (user) {
    getRepository(Series).merge(user, req.body);
    const results = await getRepository(Series).save(user);
    return res.json(results);
  }
